...
if [ -f serverstarter-2.4.2.jar ]; then
    echo "Skipping download. Using existing serverstarter-2.4.2.jar"
    java $JAVA_FLAGS -jar serverstarter-2.4.2.jar
    ...
    export URL="https://github.com/TeamAOF/ServerStarter/releases/download/v2.4.2/serverstarter-2.4.2.jar"
fi
echo $URL
which wget >> /dev/null
if [ $? -eq 0 ]; then
    echo "DEBUG: (wget) Downloading ${URL}"
    wget -O serverstarter-2.4.2.jar "${URL}"
else
    which curl >> /dev/null
    if [ $? -eq 0 ]; then
        echo "DEBUG: (curl) Downloading ${URL}"
        curl -o serverstarter-2.4.2.jar -L "${URL}"
    else
